...
const VaultView: React.FC<VaultViewProps> = ({ vault, updateStage }) => {
  ...
  return (
    <div className="p-8 h-full overflow-y-auto flex flex-col">
      <h2 className="text-4xl font-bold font-heading mb-4">My Vault</h2>
        ...
        <nav className="-mb-px flex space-x-8" aria-label="Tabs">
          {stages.map((stage) => (
            <button
              ...
              } capitalize whitespace-nowrap py-4 px-1 border-b-4 font-medium text-lg`}
            >
              {stage}
            </button>
          ))}
      ...
      <div className="flex-grow mt-6">
        {filteredVault.length === 0 ? (
          <div className="text-center py-10">
            <p className="text-xl text-gray-500">No ideas in this stage yet.</p>
            <p className="text-gray-400 mt-2">Save ideas from the 'Roll' screen to add them to your vault.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredVault.map(item => (
              <div key={item.id} className="bg-white p-5 rounded-xl shadow-md flex flex-col">
                <h3 className="text-xl font-bold font-heading">{item.idea.name}</h3>
                <p className="text-sm text-midnight-navy/70 mt-2 flex-grow">{item.idea.description}</p>
                <div className="mt-4 pt-4 border-t border-gray-200">
                    <label className="text-xs font-semibold">Stage:</label>
                    <select
                        value={item.stage}
                        onChange={(e) => updateStage(item.id, e.target.value as VaultStage)}
                        className="w-full mt-1 capitalize text-sm rounded-md p-2 border-gray-300 focus:ring-2 focus:ring-midnight-navy focus:border-transparent"
                    >
